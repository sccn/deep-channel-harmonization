...
role = 'train'
train_data = load_data_constant(path + "child_mind_abdu128/", role, winLength, 24, srate, feature, one_channel)
print(f'X_train shape: {len(train_data)}, {train_data[0][0].shape}')
#print(f'Y_train shape: {len(train_data)}, {train_data[0][1].shape}')

role = 'val'
val_data = load_data_constant(path + "child_mind_abdu128/", role, winLength, 24, srate, feature, one_channel)
